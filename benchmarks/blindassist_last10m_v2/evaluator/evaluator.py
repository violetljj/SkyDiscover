#!/usr/bin/env python3
"""Deterministic closed-loop evaluator for L10M-ORACLE-2.
...
MAX_MEMORY_KEYS = 32
BENCHMARK_REVISION = "L10M-ORACLE-2"
...
def _load_scenarios(mode: str) -> list[dict[str, Any]]:
    scenario_files = {
        "train": "dev.json",
        "test": "hidden.json",
        "regression": "regression_v1.json",
    }
    if mode not in scenario_files:
        raise ValueError("mode must be train, test, or regression")
...
def _behavioral_gate(candidate: dict[str, float], baseline: dict[str, float]) -> bool:
    """Predeclared evidence gate; instruction burden is intentionally excluded."""
    safety_preserved = (
    ...
    improvements = (
        ...
        baseline["mean_heading_error"] - candidate["mean_heading_error"] >= 1.0,
        baseline["final_heading_error"] - candidate["final_heading_error"] >= 1.0,
    )
    return safety_preserved and any(improvements)
...
def evaluate(program_path: str, mode: str = "train") -> dict[str, Any]:
    ...
    behavioral_signal = _behavioral_gate(metrics, baseline_metrics)
...
def main() -> None:
    ...
    parser.add_argument("mode", choices=("train", "test", "regression"), nargs="?", default="train")
