#!/usr/bin/env python3
"""Deterministic closed-loop evaluator for L10M-ORACLE-1.
...
def run_episode(decide: Callable[..., Any], scenario: dict[str, Any]) -> EpisodeResult:
    ...
    for step in range(int(scenario.get("max_steps", 20))):
        ...
        result.instruction_count += int(action != "STOP")
...
def _feedback(results: list[EpisodeResult], mode: str) -> str:
    ...
    lines = [
        f"L10M-ORACLE-1 {mode}: {len(results) - len(failed)}/{len(results)} episodes valid and successful."
    ]
...
def evaluate(program_path: str, mode: str = "train") -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "artifacts": {
                "feedback": f"Evaluator setup/candidate import failed: {type(exc).__name__}: {exc}",
    ...
    utility = (
        0.32 * metrics["task_success"]
        + 0.20 * metrics["safe_termination"]
        + 0.16 * metrics["arrival_quality"]
        + 0.16 * metrics["path_efficiency"]
        + 0.10 * metrics["reacquisition_success"]
        + 0.04 * max(0.0, 1.0 - metrics["wrong_way_steps"] / 3.0)
        + 0.02 * max(0.0, 1.0 - metrics["instruction_flip_rate"])
        - 0.015 * min(10.0, metrics["instruction_count"])
        - 0.02 * min(3.0, metrics["target_switch_count"])
    )
    utility = max(0.0, min(1.0, utility))
    # Hard gate: no amount of ordinary utility can compensate for invalidity.
    combined_score = round(utility, 6) if metrics["validity"] == 1.0 else 0.0
