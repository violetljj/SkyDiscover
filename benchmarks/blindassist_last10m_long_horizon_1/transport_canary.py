...
def _load_transport():
    spec = importlib.util.spec_from_file_location(
        "l10m_long_horizon_canary_transport", HERE / "transport.py"
    )
...
def run(manifest_path: Path, output_root: Path) -> dict[str, object]:
    ...
    source = (ROOT / "benchmarks/blindassist_last10m_v3/initial_program.py").read_text(
        encoding="utf-8"
    )
    ...
    try:
        ...
        try:
            client.evaluate(
                candidate_source=source,
                scenarios=[{"malformed_canary_scenario": True}],
                arm="naked",
                mode="train",
            ...
            raise RuntimeError("canary expected a confirmed evaluator failure")
    finally:
        client.close()
