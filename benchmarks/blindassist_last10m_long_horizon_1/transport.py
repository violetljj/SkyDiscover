"""Local controller client for a persistent LONG-HORIZON-1 evaluator."""
...
class RemoteEvaluationClient:
    ...
    def _command(self) -> list[str]:
        ...
        script = f"{self.manifest['source_dir']}/benchmarks/blindassist_last10m_long_horizon_1/remote_worker.py"
        worker_root = f"{self.manifest['task_root']}/workers/{self.worker_id}"
        cache_root = f"{self.manifest['task_root']}/evaluation_cache"
        return [
            "ssh",
            "-o",
            "BatchMode=yes",
