#!/usr/bin/env python3
"""Materialize the preregistered fresh L10M-SKY-1 cohort without treatment output."""
...
def _transform_scenarios(seed: int, prefix: str) -> list[dict[str, Any]]:
    ...
    generator = _load_module(f"l10m_sky1_generator_{seed}", BASE_GENERATOR)
...
def _baseline_result(raw: list[dict[str, Any]]) -> dict[str, float]:
    evaluator = _load_module(f"l10m_sky1_eval_{random.getrandbits(64)}", BASE_EVALUATOR)
...
def materialize(output_root: Path) -> dict[str, Any]:
    ...
    for instance in cohort:
        ...
        records.append(
            {
                **instance,
                "dev_path": dev_path.relative_to(ROOT).as_posix(),
...
def main() -> int:
    ...
    parser.add_argument("--output-root", type=Path, default=HERE / "cohort")
