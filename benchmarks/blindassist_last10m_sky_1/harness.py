#!/usr/bin/env python3
"""Common budgeted search and sealed adjudication harness for L10M-SKY-1."""
...
DEV_SCENARIO_ENV = "L10M_SKY1_DEV_SCENARIOS"
ARM_CONFIGS = {
    "naked_codex": HERE / "configs" / "naked_codex.yaml",
    "sky_search": HERE / "configs" / "sky_search.yaml",
}
...
def _scenario_path(record: dict[str, Any], split: str) -> Path:
    path = ROOT / record[f"{split}_path"]
...
def _load_hidden_evaluator(hidden_path: Path):
    spec = importlib.util.spec_from_file_location(
        f"l10m_sky1_hidden_{time.time_ns()}", BASE_EVALUATOR
    )
    ...
    for arm in protocol["arms"]:
        ...
        for index in range(attempts):
            if index < len(candidates) and search_receipt["status"] == "COMPLETED":
                ...
                substantive = max(
                    0.0, float(hidden["metrics"].get("substantive_score_delta", 0.0) or 0.0)
                )
            else:
                row = {
                    "attempt": index + 1,
        ...
        best_row = max(
            hidden_rows,
            key=lambda row: float(row["metrics"].get("substantive_score_delta", 0.0) or 0.0),
        )
        passes = [
            row
        ...
        arm_results[arm] = {
            ...
            "best_discovered_substantive_score_delta": best_row["metrics"].get(
                "substantive_score_delta", 0.0
            ),
