#!/usr/bin/env python3
"""Fresh-start direct generation and sealed paired adjudication harness."""
...
INITIAL_PROGRAMS = {
    "raw_direct": ROOT / "benchmarks" / "blindassist_last10m_v3" / "initial_program.py",
    "structured_direct": HERE / "structured_initial_program.py",
}
...
DEV_SCENARIO_ENV = "L10M_STRUCT_DIRECT_1_DEV_SCENARIOS"
ARM_ENV = "L10M_STRUCT_DIRECT_1_ARM"
COHORT_ROOT_ENV = "L10M_STRUCT_DIRECT_1_COHORT_ROOT"
ARM_CONFIGS = {
    "raw_direct": HERE / "configs" / "raw_direct.yaml",
    "structured_direct": HERE / "configs" / "structured_direct.yaml",
}
...
async def run_arm(arm: str, instance_id: str, seed: int, output_root: Path) -> dict[str, Any]:
    ...
    if seed not in protocol["direct_replicates"]["local_seeds"]:
        raise ValueError(f"unregistered direct replicate seed: {seed}")
    ...
    initial_program = INITIAL_PROGRAMS[arm]
...
def _load_hidden_evaluator(hidden_path: Path):
    spec = importlib.util.spec_from_file_location(
        f"l10m_struct_direct_1_hidden_{time.time_ns()}", BASE_EVALUATOR
    )
...
def _evaluate_hidden(evaluator: Any, solution: str, arm: str) -> dict[str, Any]:
    ...
    try:
        guard_spec = importlib.util.spec_from_file_location(
            f"l10m_struct_direct_1_guard_{time.time_ns()}", GUARD_PATH
        )
...
def adjudicate_block(instance_id: str, seed: int, output_root: Path) -> dict[str, Any]:
    ...
    attempts = protocol["generation_ceilings_per_arm_instance_replicate"][
        "hidden_adjudication_attempts"
    ]
    arm_results = {}
    for arm in protocol["arms"]:
        ...
        for index in range(attempts):
            if index < len(candidates) and search_receipt["status"] == "COMPLETED":
                ...
                hidden = _evaluate_hidden(evaluator, candidate["solution"], arm)
                row = {
                    "attempt": index + 1,
        ...
        best_row = max(
            hidden_rows,
            key=lambda row: (
                float(row["primary_substantive_value"]),
                bool(row["robust_safe"]),
            ),
        )
        passes = [
            row
