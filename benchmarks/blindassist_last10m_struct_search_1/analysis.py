#!/usr/bin/env python3
"""Frozen instance-level analysis for L10M-STRUCT-SEARCH-1 receipts."""
...
def analyze(receipt_root: Path) -> dict[str, Any]:
    ...
    for item in cohort["instances"]:
        ...
        for seed in seeds:
            ...
            naked = _best_score(receipt, "naked_structured")
            evox = _best_score(receipt, "evox_structured")
            sky_evox = _best_score(receipt, "sky_evox_structured")
            for arm in protocol["arms"]:
                arm_scores[arm].append(_best_score(receipt, arm))
                arm_safe[arm].append(
                    bool(receipt["arms"][arm].get("best_discovered_robust_safe", False))
                )
    ...
    delta_e = float(primary["minimum_meaningful_effect"])
    delta_h = delta_e
    absolute_margin = float(
        primary["absolute_candidate_gate"]["mean_substantive_score_delta_strictly_greater_than"]
    )
    required_safe = int(primary["absolute_candidate_gate"]["robust_safe_instances_equals"])
    absolute = {
        arm: {
            "mean_substantive_score_delta": mean(scores),
            "robust_safe_instances": sum(arm_safe[arm]),
            "qualifies": mean(scores) > absolute_margin and sum(arm_safe[arm]) == required_safe,
        }
        for arm, scores in arm_scores.items()
    }
    g0 = absolute["evox_structured"]["qualifies"] or absolute["sky_evox_structured"]["qualifies"]
    p_g1 = exact_sign_flip_greater(instance_effects_e, delta_e)
    g1 = (
        absolute["evox_structured"]["qualifies"]
        and mean(instance_effects_e) > delta_e
        and p_g1 <= alpha
    )
    ...
    if not g0:
        architecture = "STRUCTURED_TEMPORAL_SEARCHABILITY_NOT_ESTABLISHED"
    elif not g1:
        architecture = "ABSOLUTE_SEARCHABILITY_OBSERVED_INCREMENTAL_VALUE_NOT_ESTABLISHED"
    elif g2:
