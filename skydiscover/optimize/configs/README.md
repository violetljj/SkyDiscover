...
llm:
  temperature: 0.7
  top_p: null            # provider default unless set
  max_tokens: 32000
...
llm:
  models:
    - name: "gpt-5"
  evaluator_models:   # used by LLM-as-a-judge (evaluator.llm_as_judge)
    - name: "gpt-4o-mini"
...
See [`skydiscover/optimize/search/openevolve_native/README.md`](../search/openevolve_native/README.md) for architecture details.
...
prompt:
  ...
  evaluator_system_message: |   # system message for the LLM judge
    You are a strict code quality judge. ...  # only used when evaluator.llm_as_judge: true
...
evaluator:
  ...
  # null means auto: on when running from scratch (no seed program) or on a
  # Harbor task, off for seeded runs. Set true/false to force either way.
  inject_evaluator_context: null
...
monitor:
  enabled: false
  port: 8765
  host: "127.0.0.1"       # loopback by default; set 0.0.0.0 to expose the dashboard
  max_solution_length: 10000

  # AI-generated run summaries
  summary_model: null      # e.g. "gpt-5-mini"; summaries are off when unset
...
```bash
cp skydiscover/optimize/configs/evox.yaml my_config.yaml
```
...
```bash
uv run skydiscover optimize initial_program.py evaluator.py -c my_config.yaml -i 100
```
