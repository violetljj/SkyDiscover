...
from skydiscover.optimize.utils.async_utils import TaskPool
from skydiscover.optimize.utils.metrics import format_metrics
...
class ContainerizedEvaluator:
    ...
    def __del__(self):
        ...
        try:
            ...
            pass  # never raise from __del__; the container is best-effort at interpreter teardown
    ...
    def _run_single_in_container(self, candidate_path: str, mode: str) -> EvaluationResult:
        ...
        try:
            ...
            # chmod first, so a benchmark whose Dockerfile omits it does not fail on permissions.
            cmd.extend(
                [
                    self.container_id,
                    "sh",
                    "-c",
                    'chmod +x /benchmark/evaluate.sh 2>/dev/null; exec /benchmark/evaluate.sh "$1" "$2"',
                    "sh",
                    candidate_path,
                    mode,
                ]
    ...
    def _inject_file(self, content: str, suffix: str) -> str:
        ...
        try:
            inject = subprocess.run(
                ...
                capture_output=True,
                timeout=self.config.container_setup_timeout,
            )
        except subprocess.TimeoutExpired as exc:
            raise RuntimeError(
                f"Injecting {path} into the container timed out after "
                f"{self.config.container_setup_timeout}s — the Docker daemon may be wedged."
            ) from exc
    ...
    def _remove_file(self, path: str) -> None:
        ...
        try:
            ...
            # Best-effort cleanup of a temp file; the container is torn down anyway.
            logger.warning(f"Timed out removing {path} from the container; continuing.")
    ...
    def _start_container(self) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"Starting the container timed out after "
                f"{self.config.container_setup_timeout}s (image {self.image_tag})."
            ) from exc
    ...
    def _build_image(self) -> str:
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=self.config.container_build_timeout,
            )
        except subprocess.TimeoutExpired as exc:
            raise RuntimeError(
                f"Docker build for {self.benchmark_dir} timed out after "
                f"{self.config.container_build_timeout}s. Raise "
                f"evaluator.container_build_timeout if the image is genuinely this slow."
            ) from exc
