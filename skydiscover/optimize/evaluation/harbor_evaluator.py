...
class HarborEvaluator(ContainerizedEvaluator):
    ...
    def _build_image(self) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"Docker build for {dockerfile_dir} timed out after "
    ...
    def _run_container(self, program_solution: str, mode: str) -> EvaluationResult:
        ...
        try:
            ...
            logger.error(
                f"Injecting the solution timed out after " f"{self.config.container_setup_timeout}s"
            )
            return EvaluationResult(
                metrics={"combined_score": 0.0, "timeout": True},
                artifacts={"error": "timed out injecting the solution into the container"},
            )
    ...
    def _init_container(self):
        ...
        if os.path.isdir(tests_dir):
            ...
            try:
                ...
                raise RuntimeError(
                    f"Uploading tests/ to the container timed out after "
                    f"{self.config.container_setup_timeout}s."
                ) from exc
    ...
    def _read_reward(self, test_stdout: str = "", test_stderr: str = "") -> EvaluationResult:
        ...
        for path, is_json in [
            ...
            try:
                ...
                logger.warning(f"Timed out reading {path}; trying the next reward path.")
                continue
            ...
            if candidates:
                ...
                if os.path.exists(dockerfile):
                    try:
                        ...
                        pass  # unreadable or unparseable Dockerfile: keep the default base
    ...
    def _exec(self, cmd: str) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            raise RuntimeError(
                f"Container setup command timed out after "
                f"{self.config.container_setup_timeout}s: {cmd}"
            ) from exc
