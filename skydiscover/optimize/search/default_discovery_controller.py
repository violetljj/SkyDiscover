...
from skydiscover.optimize.evaluation import create_evaluator
from skydiscover.optimize.evaluation.llm_judge import LLMJudge
from skydiscover.optimize.llm.base import LLMResponse
from skydiscover.optimize.llm.codex_cli import is_codex_cli_model
...
from skydiscover.optimize.utils.code_utils import (
    apply_diff,
    extract_diffs,
    format_diff_summary,
    parse_full_rewrite,
...
class DiscoveryController:
    ...
    def __init__(self, controller_input: DiscoveryControllerInput):
        ...
        if self.config.agentic.enabled:
            from skydiscover.optimize.llm.agentic_generator import AgenticGenerator
    ...
    def _inject_evaluator_context(self):
        ...
        Controlled by ``evaluator.inject_evaluator_context``. The Runner resolves
        its ``None`` ("auto") default before the controller is built: on for
        from-scratch and Harbor runs, off for seeded runs. A still-unset value
        here (e.g. the co-evolution search side, built without a Runner) is
        treated as off.
        """
        if not self.config.evaluator.inject_evaluator_context:
            return

        from skydiscover.optimize.search.utils.discovery_utils import load_evaluator_code
