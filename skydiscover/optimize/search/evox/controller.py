...
from skydiscover.optimize.search.evox.utils.search_scorer import LogWindowScorer
from skydiscover.optimize.search.evox.utils.variation_operator_generator import (
    generate_variation_operators,
)
from skydiscover.optimize.search.registry import get_program, setup_search
...
class CoEvolutionController(DiscoveryController):
    ...
    async def _generate_variation_operators(self) -> None:
        ...
        if not getattr(db_cfg, "auto_generate_variation_operators", True):
            from skydiscover.optimize.search.evox.utils.template import (
