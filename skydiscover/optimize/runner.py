...
from skydiscover.optimize.search.route import get_discovery_controller
from skydiscover.optimize.search.utils.logging_utils import setup_search_logging
from skydiscover.optimize.utils.code_utils import extract_solution_language
from skydiscover.optimize.utils.metrics import format_metrics, get_score
...
class Runner:
    ...
    def __init__(
        ...
        # Resolve inject_evaluator_context "auto" (None): without a seed program the
        # model has nothing but the evaluator to learn the task from, and a Harbor
        # task's instruction.md exists precisely to be shown to the model. Seeded
        # non-Harbor runs keep the conservative default of not leaking evaluator
        # internals into the prompt.
        if self.config.evaluator.inject_evaluator_context is None:
            from skydiscover.optimize.evaluation import _is_harbor_task
    ...
    @property
    def _controller(self) -> DiscoveryController:
        """The controller for the run in flight.

        ``discovery_controller`` is None before ``run()`` builds one and again
        after it is closed, so callers that only make sense mid-run go through
        here and get a clear error instead of an AttributeError on None.
        """
        if self.discovery_controller is None:
            raise RuntimeError("No active discovery controller — this is only valid during run().")
        return self.discovery_controller
    ...
    async def _add_initial_program(self, start_iteration: int) -> None:
        ...
        # run() only calls this once it has established there is a seed solution;
        # bind it locally so that stays true for readers and type checkers alike.
        solution = self.initial_program_solution
        if solution is None:
            raise RuntimeError("_add_initial_program requires a seed solution")
        ...
        if self.config.language == "image":
            ...
            try:
                result = await self._controller.llms.generate(
        ...
        eval_result = await self._controller.evaluator.evaluate_program(eval_input, program_id)
        metrics = eval_result.metrics

        image_path_metric = metrics.get("image_path")
        if not initial_image_path and isinstance(image_path_metric, str):
            initial_image_path = image_path_metric
    ...
    def _start_monitor(self, max_iterations: int):
        if not self.config.monitor.enabled:
            return None
        try:
            from skydiscover.optimize.extras.monitor import MonitorServer, create_monitor_callback
            ...
            self._controller.monitor_callback = callback
    ...
    def _setup_human_feedback(self, monitor_server) -> None:
        if not (self.config.human_feedback_enabled or monitor_server):
            return
        try:
            from skydiscover.optimize.context_builder import HumanFeedbackReader
            ...
            self._controller.feedback_reader = reader
            if monitor_server:
    ...
    def _load_initial_program(self) -> str:
        # Only called from __init__ when initial_program_path is set.
        if self.initial_program_path is None:
            raise RuntimeError("_load_initial_program requires initial_program_path")
