...
class ConfigError(ValueError):
    """A config file named a key SkyDiscover does not understand.

    Raised instead of the dataclass's bare ``TypeError: __init__() got an
    unexpected keyword argument``, which names the key but not where it sat or
    what was valid there.
    """


def _reject_unknown_keys(section: Optional[str], given: Iterable[str], valid: Set[str]) -> None:
    """Fail with a message that says where the key was and what was meant instead."""
    unknown = [k for k in given if k not in valid]
    if not unknown:
        return

    where = f"the '{section}' section" if section else "the top level"
    lines = []
    for key in unknown:
        near = get_close_matches(key, sorted(valid), n=1, cutoff=0.6)
        suggestion = f" — did you mean '{near[0]}'?" if near else ""
        lines.append(f"  {key}{suggestion}")

    raise ConfigError(
        f"Unrecognised key(s) at {where} of your config:\n"
        + "\n".join(lines)
        + f"\n\nValid keys at {where}: "
        + ", ".join(sorted(valid))
    )
...
def _build_section(cls: Any, values: Dict[str, Any], section: Optional[str] = None) -> Any:
    """Instantiate a config dataclass, naming unknown keys rather than raising TypeError."""
    _reject_unknown_keys(section, values, {f.name for f in fields(cls)})
    return cls(**values)
    ...
    def __post_init__(self):
        ...
        for model in self.models + self.evaluator_models + self.guide_models:
            if model.name and model.api_base is None:
                ...
                # Strip a recognized provider prefix so the API receives the bare model name
                if provider is not None and provider != "codex-cli" and "/" in model.name:
...
class EvaluatorConfig:
    ...
    timeout: int = 360
    # Docker lifecycle bounds (seconds). ``timeout`` above bounds the evaluation
    # itself; these bound the setup around it, so a stalled image build or a wedged
    # daemon fails the run instead of hanging it forever. Builds get a far longer
    # budget than admin calls because they may pull and compile layers.
    container_build_timeout: int = 1800
    container_setup_timeout: int = 120
    max_retries: int = 3
    ...
    # exactly how solutions are scored.  None means auto, resolved by the
    # Runner: inject when there is no seed program (the model has nothing
    # else to learn the task from) or for Harbor tasks (instruction.md is
    # the task description); stay off for seeded runs to avoid leaking
    # implementation details that may introduce noise.
    inject_evaluator_context: Optional[bool] = None
...
class DatabaseConfig:
    ...
    # which is what every engine did before this was declared. Engines that seed take
    # it from here; GEPA and OpenEvolve-native already did, under their own copy.
...
class Config:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> Config:
        ...
        # Update top-level fields. Sections are handled below; anything else must be a
        # real field -- silently dropping unknown keys let a config ask for something
        # (a seed, human feedback) and get nothing, with no way to tell.
        sections = {
            "llm",
            "prompt",
            "database",
        ...
        top_level = {k: v for k, v in config_dict.items() if k not in sections}
        _reject_unknown_keys(None, top_level, {f.name for f in fields(cls)} | sections)
        ...
        if "llm" in config_dict:
            ...
            # Model entries get the same treatment, indexed so the message points at
            # the offending entry rather than the whole list.
            for list_field in ("models", "evaluator_models", "guide_models"):
                if list_field in llm_dict:
                    llm_dict[list_field] = [
                        _build_section(LLMModelConfig, entry, f"llm.{list_field}[{i}]")
                        for i, entry in enumerate(llm_dict[list_field])
                    ]
            config.llm = _build_section(LLMConfig, llm_dict, "llm")
        if "prompt" in config_dict:
            config.context_builder = _build_section(
                ContextBuilderConfig, config_dict["prompt"], "prompt"
            )
        ...
        if "search" in config_dict:
            ...
            if "database" in search_dict:
                ...
                # Split into declared fields and unknown keys; unknown keys are
                # kept for forward-compat but usually mean a typo, so warn.
                ...
                if db_extras:
                    logger.warning(
                        "search.database has unknown keys for %s: %s (kept as-is; check for typos)",
                        db_config_cls.__name__,
                        ", ".join(sorted(db_extras)),
                    )
            ...
            config.search = _build_section(SearchConfig, search_dict, "search")

        if "evaluator" in config_dict:
            config.evaluator = _build_section(
                EvaluatorConfig, config_dict["evaluator"], "evaluator"
            )
        if "agentic" in config_dict:
            ...
            config.agentic = _build_section(AgenticConfig, agentic_dict, "agentic")
        ...
        if "monitor" in config_dict:
            config.monitor = _build_section(MonitorConfig, config_dict["monitor"], "monitor")
...
def load_config(config_path: Optional[Union[str, Path]] = None) -> Config:
    ...
    if not config.llm.api_key:
        ...
        if config.llm.models:
            ...
            if first_model_name:
                provider, _, _, env_vars = _parse_model_spec(first_model_name)
        api_key = _resolve_api_key_from_env(env_vars, provider)
        if api_key:
...
def apply_overrides(
    ...
    *,
    ...
    if not model and api_base:
        effective_api_base = config.llm.api_base or api_base
        parsed_env_vars: Optional[List[str]] = None
        for _prefix, (base_url, env_list) in _PROVIDERS.items():
            if base_url and effective_api_base.startswith(base_url.rstrip("/")):
